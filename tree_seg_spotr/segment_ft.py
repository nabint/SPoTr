...
# Data utilities
def _augment_positions_numpy(pos: np.ndarray) -> np.ndarray:
    """
    Apply provider augmentations to positions stored as numpy arrays.
    The provider functions expect shape [B, N, 3] numpy arrays.
    Returns the augmented numpy array.
    """
...
# # kept for compatibility / possible criterion that expects one-hot cls
# def to_categorical(label: torch.Tensor, num_classes: int) -> torch.Tensor:
#     """
#     Convert a label tensor [B] or [B,1] to one-hot [B, num_classes].
#     Kept for compatibility with parts of the code that may want one-hot cls.
#     """
#     if label.dim() == 2:
#         label = label[:, 0]
...
def train_one_epoch_openpoints(
    ...
    for points, label, target, data_chosen, filename in train_loader:
        ...
        # Collapse earlier and keep only P
...
def evaluate_openpoints(
    ...
    """
    Evaluate model on test_loader and compute:
      - overall accuracy (per-point)
      - class avg accuracy
      - class average IoU (per category mean)
      - instance average IoU (mean over all samples)
      - per-category IoU dictionary
    """
    ...
    with torch.no_grad():
        ...
        for batch_id, (points, label, target, data_chosen, filename, *rest) in pbar:
            ...
            # print(f"Batch accuracy {correct / (B * N)}")
...
def main_training_loop(cfg: EasyConfig, seg_classes: dict):
    """
    High-level training loop. Mirrors original behaviour, but structured.
    Uses cfg fields (some may be filled from CLI via apply_args_to_cfg).
    """

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print("Device:", device)
    # cfg.model.cls_args.num_classes = 4
...
def apply_args_to_cfg(cfg: EasyConfig, args: argparse.Namespace) -> EasyConfig:
    """
    Merge CLI args into cfg only when a value is missing in cfg.
    """
    scalar_fields = [
        "model",
        "batch_size",
    ...
    if not hasattr(cfg, "num_classes"):
        cfg.num_classes = 4  # 1 Standing + 3 Fallen
    if not hasattr(cfg, "num_part"):
        cfg.num_part = 4  # Unique Part same as above
...
def main():
    ...
    # apply fallbacks where config is missing
